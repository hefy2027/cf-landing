...
function syncHtmlLang() {
  ...
  if (locale.value === 'zh-CN') {
    ...
    if (titleMeta) titleMeta.setAttribute('content', 'CF Manager 是面向开发者与运维的一站式 Cloudflare 多账户统一管理平台：可视化管理 DNS、Workers、Pages、KV/D1/R2 存储，内置 AI 推理与浏览器渲染，并暴露 OpenAI 兼容 API。支持 Cloudflare Pages 与 Docker 双部署。')
    ...
    if (titleMeta) titleMeta.setAttribute('content', 'CF Manager is an all-in-one multi-account Cloudflare management platform for developers and ops teams: visual DNS, Workers, Pages, KV/D1/R2 storage management, built-in AI inference & browser rendering, with OpenAI-compatible API. Supports Cloudflare Pages & Docker deployment.')
